...
export const DEFAULT_CORE_CONFIG: MemoryCoreConfig = {
  diaryExtractEvery: 8,
...
const ENVIRONMENT_EVIDENCE_PATTERN =
  /network|timeout|timed out|EPERM|EACCES|ECONNREFUSED|ENOTFOUND|429|503|quota|rate.?limit|unauthorized|dns|proxy|网络|超时|断网|权限|配额|限流|服务不可用|服务繁忙/i
 ...
 */
const INGEST_SOURCE_PRIOR: Record<IngestSourceType, { alpha: number; beta: number }> = {
...
export class MemoryCore {
  ...
  constructor(private readonly store: MemoryStore, readonly config: MemoryCoreConfig) {}
  ...
  refine(request: RefineExperienceRequest, actor: MemoryActor): RefineExperienceResult {
    ...
    if (request.failureReason !== undefined) snapshot.failureReason = request.failureReason
  ...
  ingest(request: IngestRequest, actor: MemoryActor): IngestResult {
    ...
    const accepted: ExperienceSnapshot[] = []
    const rejected: { gist: string; reason: string }[] = []
    const acceptedDedup: Set<string>[] = []
    ...
    for (const item of request.experiences) {
      ...
      if (item.failureReason !== undefined) snapshot.failureReason = item.failureReason
      this.store.upsertExperience(snapshot)
  ...
  recall(request: RecallExperiencesRequest, actor: MemoryActor): RecallExperiencesResult {
    ...
    let candidateTrials: CandidateTrial[] = []
    ...
    const adjudicated: RecalledExperience[] = []
    for (const { snapshot, score } of candidates) {
      ...
      for (const limit of snapshot.limits) {
        ...
        if (hits / limitTokens.length >= 0.5) conflicts.push(limit)
      }
    ...
    if (adjudicated.length === 0) {
      const none: RecallExperiencesResult = {
        items: [],
        none: true,
    ...
    const ranked = [...adjudicated].sort((a, b) => {
      const value = (item: RecalledExperience): number => {
    ...
    const items: RecalledExperience[] = []
    let usedTokens = 0
    let omitted = 0
    for (const item of ranked) {
      const cost = estimateTokens(JSON.stringify(item.experience))
      if (usedTokens + cost > budget && items.length > 0) { omitted += 1; continue }
      items.push(item)
      usedTokens += cost
    }
  ...
  report(request: ReportUseRequest, actor: MemoryActor): ReportUseResult {
    ...
    if (target === undefined) throw new Error(`memory: experience not found: ${request.id}`)

    const base: ReportUseResult = {
      snapshot: target,
      counted: 'none',
    ...
    let attribution: ReportUseResult['attributionApplied']
    ...
    const counted: ReportUseResult['counted'] = request.outcome === 'success'
      ? 'alpha'
    ...
    if (!inserted) {
      if (overrideNote !== undefined) base.overrideNote = overrideNote
      return base
    }

    let next: ExperienceSnapshot = { ...target, updatedAt: Date.now() }
    if (counted === 'alpha') {
  ...
  private shouldChallenge(exp: ExperienceSnapshot): boolean {
    const reports = this.store.reportsFor(exp.id, exp.revision, this.config.challengeWindow)
    let consecutive = 0
    for (const report of reports) {
      if (report.outcome === 'fail' && report.attribution === 'experience') consecutive += 1
      else break
    }
    ...
    if (reports.length >= 3) {
      ...
      if (fails / reports.length >= this.config.challengeWindowFailRate) return true
    }
    return false
  }

  /** Close the parent revision as superseded after a draft adoption. */
  private supersedeParent(draft: ExperienceSnapshot, actor: MemoryActor): void {
    if (draft.parentRevision === undefined) return
  ...
  private enforceFamilyCap(familyTag: string, actor: MemoryActor): void {
  ...
  revise(request: ReviseExperienceRequest, actor: MemoryActor): ExperienceSnapshot {
    ...
    const now = Date.now()
    const draft: ExperienceSnapshot = {
      ...current,
  ...
  verifyShadow(request: VerifyShadowRequest, actor: MemoryActor): VerifyShadowResult {
    ...
    if (draft === undefined) throw new Error(`memory: draft not found: ${request.id} v${String(request.revision)}`)
    if (draft.status !== 'candidate') {
      throw new Error(`memory: shadow replay verifies candidate drafts (status=${draft.status})`)
    }
    if (request.samples.length === 0) return { passed: false, agreement: 0, reason: 'no samples supplied' }
    ...
    for (const sample of request.samples) {
      ...
      if (predictsSuccess === (sample.expected === 'success')) matched += 1
    }
    ...
    const adopted: ExperienceSnapshot = { ...draft, status: 'live', lastVerifiedAt: Date.now(), updatedAt: Date.now() }
  ...
  rollback(request: RollbackExperienceRequest, actor: MemoryActor): ExperienceSnapshot {
    ...
    if (target === undefined) throw new Error(`memory: revision not found: ${request.id} v${String(request.toRevision)}`)
    ...
    const restored: ExperienceSnapshot = { ...target, status: 'live', updatedAt: Date.now() }
  ...
  appendDiary(request: DiaryAppendRequest, actor: MemoryActor): DiaryAppendResult {
    ...
    if (request.sessionRef !== undefined) entry.sessionRef = request.sessionRef
    ...
    if (!extractionDue) return { entry, extractionDue: false }
  ...
  extract(request: ExtractFactsRequest, actor: MemoryActor, trigger: 'cadence' | 'manual' = 'manual'): ExtractFactsResult {
    ...
    for (const proposal of request.proposals) {
      ...
      if (current !== undefined && current.value === proposal.value) {
        ...
        const corroborated: FactEntry = { ...current, corroboration: current.corroboration + 1, sourceDiaryIds: merged }
    ...
    for (const cp of request.concerns ?? []) {
      ...
      if (cp.action === 'new') {
        ...
        if (cp.concernId === undefined || (cp.title ?? '').trim() === '') continue
        const mention: ConcernEntry = {
          id: randomUUID(),
        ...
        if (cp.concernId === undefined || cp.status === undefined) continue
  ...
  private validateProposal(proposal: FactProposal): string | undefined {
    ...
    for (const id of proposal.sourceDiaryIds) {
      ...
      if (entry === undefined) return `unknown diary entry: ${id}`
  ...
  listConcerns(kind: string | undefined, status: string | undefined, limit?: number, offset?: number): ConcernTree[] {
    const filter: { kind?: string; status?: string } = {}
   ...
   */
  consolidationDue(): { due: boolean; lastTs: number; newSince: number; hoursSince: number } {
  ...
  consolidate(request: ConsolidateRequest, actor: string): ConsolidateResult {
    const now = Date.now()
    const result: ConsolidateResult = { consolidated: 0, archived: 0, skipped: [] }
    for (const merge of request.merges) {
      ...
      const lastVerifiedAt = sources.reduce<number | undefined>(
        (max, s) => (s.lastVerifiedAt !== undefined && (max === undefined || s.lastVerifiedAt > max) ? s.lastVerifiedAt : max),
        undefined,
      )
      const consolidated: ExperienceSnapshot = {
        id: randomUUID(),
        revision: 1,
        kind: merge.kind,
        source: 'agent',
  ...
  humanPin(request: HumanPinRequest, actor: MemoryActor): ExperienceSnapshot {
    const current = this.store.getActiveRevision(request.id)
    if (current === undefined) throw new Error(`memory: experience not found: ${request.id}`)
    const next: ExperienceSnapshot = { ...current, pinned: request.pinned, updatedAt: Date.now() }
    this.store.upsertExperience(next)
  ...
  humanPromote(id: string, reason: string, actor: MemoryActor): ExperienceSnapshot {
    ...
    if (current === undefined) throw new Error(`memory: experience not found: ${id}`)
  ...
  humanReleaseCold(request: HumanReleaseColdRequest, actor: MemoryActor): ExperienceSnapshot {
    ...
    const next: ExperienceSnapshot = { ...cold, status: 'candidate', updatedAt: Date.now() }
  ...
  humanAckDiary(request: HumanAckDiaryRequest, actor: MemoryActor): DiaryEntry {
    ...
    if (entry === undefined) throw new Error(`memory: unknown diary entry: ${request.diaryId}`)
  ...
  humanEditFact(request: HumanEditFactRequest, actor: MemoryActor): FactEntry {
    ...
    if (current === undefined) throw new Error(`memory: fact not found: ${request.factId}`)
    if (current.validTo !== undefined || current.deleted) throw new Error('memory: only the current fact version can be edited')
    const now = Date.now()
    const version: FactEntry = {
      ...current,
      id: randomUUID(),
  ...
  humanDeleteFact(request: HumanDeleteFactRequest, actor: MemoryActor): void {
    const current = this.store.getFact(request.factId)
    if (current === undefined) throw new Error(`memory: fact not found: ${request.factId}`)
    if (current.validTo !== undefined || current.deleted) throw new Error('memory: only the current fact version can be deleted')
  ...
  humanConfirmFact(request: HumanConfirmFactRequest, actor: MemoryActor): FactEntry {
    const current = this.store.getFact(request.factId)
    if (current === undefined) throw new Error(`memory: fact not found: ${request.factId}`)
    const next: FactEntry = { ...current, locked: request.locked, conflictPending: false }
  ...
  stats(): MemoryStats {
    ...
    const byStatus: MemoryStats['experiences']['byStatus'] = {
      candidate: 0, live: 0, challenged: 0, superseded: 0, archived: 0, cold: 0,
    }
    const byKind: MemoryStats['experiences']['byKind'] = { positive: 0, negative: 0 }
    let trustSum = 0
    let pinned = 0
