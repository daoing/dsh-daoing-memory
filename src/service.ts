...
const DEDUP_STATUSES: ExperienceStatus[] = ['candidate', 'live', 'challenged', 'archived', 'cold']
...
export class MemoryService extends TypertRemoteService {
  ...
  @Remote('refine')
  async refine(agent: Agent, request: RefineExperienceRequest): Promise<RefineExperienceResult> {
    const actor = actorOf(agent)
    const cands = this.core.nearDuplicateCandidates(
      request.gist,
      request.situation,
      DEDUP_STATUSES,
      DEDUP_TOP_K,
  ...
  @Remote('ingest')
  async ingest(agent: Agent, request: IngestRequest): Promise<IngestResult> {
    const actor = actorOf(agent)
   ...
   */
  private async dedupVerdictWithLlm(
    agent: Agent,
    lesson: { gist: string; situation: string[]; path: ExperienceStep[]; reasoning: string; limits: string[] },
    cands: { snapshot: ExperienceSnapshot; score: number }[],
  ): Promise<'merge' | 'duplicate' | 'different'> {
    ...
    if (provider === undefined || model === undefined) {
      ...
      if (sel !== undefined) { provider = sel.provider; model = sel.model }
    }
    if (provider === undefined || model === undefined) return 'different'
    if (this.llm === undefined) return 'different'

    const fmtLesson = (l: { gist: string; situation: string[]; path: ExperienceStep[]; reasoning: string; limits: string[] }) =>
      'gist: ' + l.gist + '\nsituation: ' + l.situation.join(' | ') + '\npath:\n' +
      l.path.map((p) => '  ' + p.order + '. ' + p.action).join('\n') +
      '\nreasoning: ' + l.reasoning + '\nlimits: ' + l.limits.join(' | ')

    const candText = cands.map((c, i) =>
      '候选 ' + (i + 1) + '（词面相似度 ' + c.score.toFixed(3) + '）：\n' + fmtLesson(c.snapshot),
    ).join('\n\n')
