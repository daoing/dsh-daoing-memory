...
等价写法也可：`git+https://github.com/daoing/daoing-dsh-memory.git`，可用 `#<分支|标签|提交>` 固定版本。
...
```sh
dsh plugin --profile web add daoing-dsh-memory        # 从 npm
# 或
dsh plugin --profile web add github:daoing/daoing-dsh-memory   # 从 GitHub
```
...
> **关于内置 memory 行。** 源码 monorepo 的 base bundle 已内置 memory 行（`id: memory`、`id: memory-tools`）。profile patch 采用*按行 id 后写覆盖*，因此安装 `daoing-dsh-memory` 会把这些已有行**重新指向** `daoing-dsh-memory` 实现，而不是新增一份。如果你想并行对比，可在本地 `cordis.patch.yml` 里给行改名。
...
- **`dsh plugin` 提示该包“未声明 dsh.bundle”** —— 你装的不对（或装到了陈旧构建）。确认装的是 `daoing-dsh-memory`，且其 `package.json` 声明了 `dsh.bundle.patch`。
- **重启后没有 Memory 分区** —— 检查 profile 的 bundle 列表是否包含 `daoing-dsh-memory`（CLI 安装时会自动对齐）。完整重启 DSH。
