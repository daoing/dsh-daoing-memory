...
var MemoryStore = class {
    ...
    `);
		...
		if (row === void 0) db.prepare("INSERT INTO memory_meta (key, value) VALUES (?, ?)").run("schema_version", String(7));
		else {
        ...
        `);
			...
			if (v !== 7) throw new Error(`memory store schema version ${row.value} does not match 7; refusing to open`);
			db.prepare("UPDATE memory_meta SET value = ? WHERE key = ?").run(String(7), "schema_version");
		}
...
var MemoryCore = class MemoryCore {
	...
	*  and non-live (challenged/cold/archived) are rejected. */
	evolve(request, actor) {
		const current = this.store.getActiveRevision(request.id);
		if (current === void 0) throw new Error(`memory: experience not found: ${request.id}`);
		if (current.approvedBy !== "human") throw new Error(`memory: only human-approved experiences evolve (approvedBy=${String(current.approvedBy)})`);
		if (current.status !== "live") throw new Error(`memory: only live experiences evolve (status=${current.status})`);
		const situation = request.situation === void 0 ? current.situation : [...new Set([...current.situation, ...request.situation])];
		const limits = request.limits === void 0 ? current.limits : [...new Set([...current.limits, ...request.limits])];
		const now = Date.now();
		const next = {
			...current,
			revision: current.revision + 1,
		...
		this.store.upsertExperience(next);
		this.supersedeParent(next, actor);
		this.ledger("evolve", "experience", next.id, actor, {
			revision: next.revision,
			by: current.revision
		}, request.reason);
		return next;
...
let MemoryService = (() => {
	...
	return class MemoryService extends _classSuper {
		static {
			...
			__esDecorate(this, null, _evolve_decorators, {
				kind: "method",
				name: "evolve",
				static: false,
				private: false,
				access: {
					has: (obj) => "evolve" in obj,
					get: (obj) => obj.evolve
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
...
//#endregion
export { DEFAULT_CORE_CONFIG, MEMORY_SCHEMA_VERSION, MemoryCore, MemoryService, MemoryStore, apply, inject, resolveConfig };
