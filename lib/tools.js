...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		description: "特殊通道：把一条通用经验直接注入经验库（source=human、status=live、信任地板，立即可被召回）。在 UI 人工管理页不便操作时使用；写入会 append 到审计账本（actor=human），账本可校验，不破坏 hash 链。一次调用注入一条，须写成可迁移的通用教训（reasoning 讲 why 与边界，勿写「本会话/本次」流水账）。",
		parameters: {
			kind: {
				type: "string",
				required: true,
				enum: ["positive", "negative"],
				description: "positive=可用路径; negative=确认的路径."
			},
			family: {
				type: "string",
				required: true,
			...
			gist: {
				type: "string",
				required: true,
				description: "一条可复用的通用教训."
			},
			situation: {
				type: "array",
				required: true,
				items: { type: "string" },
				description: "触发召回的情境类别(可迁移), 非「本任务」."
			},
			path: {
				type: "array",
				required: true,
			...
			reasoning: {
				type: "string",
				required: true,
				description: "可迁移的 why (在<situation>下成立因<cause>); 非会话叙述."
			},
			limits: {
				type: "array",
				required: true,
				items: { type: "string" },
				description: "此经验不适用的边界."
			},
			failureReason: {
				type: "string",
				description: "negative 注入的失败原因."
			},
			context: {
				type: "string",
				description: "上下文/域作用域."
			},
			reason: {
				type: "string",
				required: true,
