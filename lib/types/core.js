...
import { isSystemExperienceFamily } from "./types.js";
...
export class MemoryCore {
    ...
    report(request, actor) {
        ...
        if (request.outcome === 'success' && attribution === 'experience') {
            if (next.status === 'candidate' && next.parentRevision !== undefined) {
                ...
                const adopted = { ...next, status: 'live', approvedBy: 'agent' };
                ...
                next = { ...next, status: 'live', approvedBy: 'agent' };
    ...
    createSkillDraft(experienceId, form, content, draftPath, actor) {
        ...
        if (experience === undefined)
            throw new Error(`memory: experience not found: ${experienceId}`);
        const now = Date.now();
        const id = crypto.randomUUID();
    ...
    isSkillCandidate(experienceId) {
        ...
        if (exp === undefined || exp.status !== 'live')
            return false;
        if (exp.path.length < 3)
            return false; // too simple
        if (exp.verifiedCount < 2)
            return false; // not enough verified uses
        ...
        if (existing.some(a => a.status === 'published' || a.status === 'approved' || a.status === 'pending_review'))
            return false;
        return true;
    }
    ...
    humanDeleteExperience(request, actor) {
        ...
        if (isSystemExperienceFamily(head.family)) {
            throw new Error(`memory: system experience "${head.family}" cannot be deleted (only archived). `
    ...
    humanPromote(id, reason, actor) {
        ...
        const next = { ...current, status: 'live', approvedBy: 'human', lastVerifiedAt: Date.now(), updatedAt: Date.now() };
        this.store.upsertExperience(next);
