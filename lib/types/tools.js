 ...
 *
 * @module daoing-dsh-memory/tools
 */
...
function renderRecalled(result) {
    if (result.none)
        return `【负向通道】无相关经验：${result.reason ?? '经验库中没有匹配项'}（这是一次全新探索）`;
...
const RECALL_DESCRIPTION = 'Recall earned experiences for a similar situation. Call this BEFORE starting a complex task. '
...
const REPORT_DESCRIPTION = '用·验: report one use outcome for an experience — using it IS verifying it (V0). Call this AFTER '
...
const REPORT_OUTPUT = {
    ...
    render: (_args, value) => {
        const notes = [];
        if (value.counted === 'alpha')
            notes.push('计入成功样本 α+1');
        else if (value.counted === 'beta')
            notes.push('计入失败样本 β+1');
        else
            notes.push('未计入（环境失败/信号不足/幂等去重）');
        if (value.overrideNote !== undefined)
            notes.push(value.overrideNote);
        if (value.challenged)
            notes.push('⚠️ 已隔离（challenged，移出召回集，需修订）');
        if (value.adopted)
            notes.push('修订版通过验证转正（旧版转 superseded 可回滚）');
        else if (value.promoted)
            notes.push('通过验证转正为 live');
        return [{ type: 'text', text: `经验 [${value.id}] ${value.status} ${trustTripleShort(value)} — ${notes.join('；')}` }];
    },
};
...
const REVISE_DESCRIPTION = '修: propose a revised draft (v+1) for a CHALLENGED experience. The challenged original is already '
...
const VERIFY_DESCRIPTION = '修·V1: controlled re-enactment (shadow replay) for a revised draft: replay historical situations '
...
const FACT_DESCRIPTION = '记: append one diary entry about an important interaction with the user (event layer — permanent, '
...
const EXTRACT_DESCRIPTION = '记·上升通道: distill the pending diary window into (a) profile facts and (b) concern memos. '
...
const LEDGER_DESCRIPTION = 'Query the memory audit ledger (append-only event sourcing: every refine/use/challenge/revision/'
...
const LEDGER_OUTPUT = {
    ...
    render: (_args, value) => [{
            ...
            text: value.blocks.length === 0
                ? '账本中没有匹配的记录。'
                : value.blocks.map(b => `#${String(b.seq)} ${new Date(b.ts).toISOString().slice(0, 16).replace('T', ' ')} ${b.op} ${b.objectType}:${b.objectId} by ${b.actor}${b.reason !== undefined ? `（${b.reason}）` : ''}`).join('\n'),
        }],
};
// ── memory_consolidate (008 §1): periodic merge of related experiences ──────
const CONSOLIDATE_DESCRIPTION = '记·巩固: merge a set of closely related experiences into ONE more distilled experience. Use when '
